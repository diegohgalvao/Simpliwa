/*
  # Fix Database Functions and Triggers for User Signup

  This migration fixes the database error during user signup by properly handling
  function dependencies and recreating all necessary functions and policies.

  1. Drop existing functions with CASCADE to handle dependencies
  2. Recreate all functions with correct signatures
  3. Recreate any dropped policies
  4. Set up proper triggers and permissions
*/
...
DROP FUNCTION IF EXISTS public.user_is_company_admin(uuid) CASCADE;
DROP FUNCTION IF EXISTS public.user_is_super_admin() CASCADE;
DROP FUNCTION IF EXISTS public.user_can_create_company() CASCADE;
DROP FUNCTION IF EXISTS public.user_has_permission(uuid, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_company_member() CASCADE;
DROP FUNCTION IF EXISTS public.handle_company_member_role_change() CASCADE;
DROP FUNCTION IF EXISTS public.prevent_super_admin_company_membership() CASCADE;
...
-- Recreate any policies that may have been dropped
DO $$
BEGIN
  -- Recreate companies policies if they don't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'companies' AND policyname = 'users_can_read_own_companies'
  ) THEN
  ...
  -- Recreate company_members policies if they don't exist
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'company_members' AND policyname = 'users_can_view_company_members'
  ) THEN
