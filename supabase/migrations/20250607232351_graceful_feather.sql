/*
  # Fix Function Dependencies and RLS Policies

  This migration properly handles the dependencies between RLS policies and functions
  by dropping policies first, then functions, then recreating everything in the correct order.

  1. Drop dependent RLS policies
  2. Drop existing functions
  3. Recreate functions with proper definitions
  4. Recreate RLS policies
  5. Set up triggers and permissions
*/

-- First, drop all dependent RLS policies
...
-- Now recreate the RLS policies that depend on these functions
