...
DROP TRIGGER IF EXISTS on_company_member_insert ON company_members;
DROP TRIGGER IF EXISTS on_company_member_update ON company_members;
DROP TRIGGER IF EXISTS prevent_super_admin_membership ON company_members;

-- Then drop ALL dependent RLS policies across all tables
...
-- Create the trigger (after functions are created)
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Create triggers for company member management
CREATE TRIGGER on_company_member_insert
  AFTER INSERT ON company_members
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_company_member();

CREATE TRIGGER on_company_member_update
  AFTER UPDATE ON company_members
  FOR EACH ROW EXECUTE FUNCTION public.handle_company_member_role_change();

CREATE TRIGGER prevent_super_admin_membership
  BEFORE INSERT ON company_members
  FOR EACH ROW EXECUTE FUNCTION public.prevent_super_admin_company_membership();
