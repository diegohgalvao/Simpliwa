/*
  # Fix RLS Policies and Remove Recursion Issues

  1. Security Functions
    - Create SECURITY DEFINER functions to prevent RLS recursion
    - Functions check user permissions without triggering recursive policy checks

  2. Updated Policies
    - Replace recursive policies with function-based policies
    - Fix profile policies to allow proper user management
    - Ensure company access control works correctly

  3. Performance
    - Add necessary indexes for better query performance
    - Grant proper permissions on security functions
*/
...
-- Note: Role changes are handled separately by super admin policy
